        ...
        ImmutableArray<RecordDeclarationSyntax> recordDeclarations)
    {
        foreach (var recordDeclaration in recordDeclarations)
        {
            ...
            foreach (var interfaceDeclaration in root.DescendantNodes().OfType<InterfaceDeclarationSyntax>())
            {
                ...
                foreach ((string _, string propertyName) in propertyDescriptors)
                {
                    sb.Append("            ");
                    sb.Append(propertyName);
                    sb.Append(" = ");

                    var parameterName = char.ToLower(propertyName[0]) + propertyName[1..];
                    sb.Append($"{parameterName}.HasValue ? {parameterName}.Value : {propertyName}");
